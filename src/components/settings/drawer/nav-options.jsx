...
export function LayoutOption({ option, selected, sx, ...other }) {
  const renderIconNav = () => {
    ...
    const primaryLine = (
      <Box
        sx={{
          ...
          height: 4,
          opacity: 0.48,
          ...(option === 'hide' ? {width: 0} : option === 'horizontal' && { width: 16 }),
    ...
    const secondaryLine = (
      <Box
        sx={{
          ...
          maxWidth: 14,
          opacity: 0.24,
          ...(option === 'hide' ? {maxWidth: 0} : option === 'horizontal' && { maxWidth: 10 }),
          ...(selected && { background: 'var(--active-color)' }),
        }}
      />
    );
    ...
    return (
      <Box
        ...
        flexShrink={0}
        display="flex"
        flexDirection={option === 'horizontal' || option === 'hide' ? 'row' : 'column'}
        sx={{
          p: 0.75,
          width: 32,
          height: 1,
  ...
  const renderIcon = (
    <Box
      display="flex"
      flexDirection={option === 'horizontal' || option === 'hide' ? 'column' : 'row'}
      sx={(theme) => ({
        width: 1,
        height: 1,
